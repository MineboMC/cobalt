...
public class BooleanTraitButton<T> extends Button {
    ...
    public ItemStack build() {
        ...
        if (meta != null) {
            ...
            meta.setDisplayName(Coloring.translateColors(statusColor + "<bold>" + traitName));

            List<String> lore = new ArrayList<>();
            lore.add("");
            lore.add(Coloring.translateColors("<gray>Status: " + (currentValue ? "<green>Enabled" : "<red>Disabled")));
            lore.add("");
            lore.add(Coloring.translateColors("<yellow>Click to " + (currentValue ? "disable" : "enable")));
    ...
    private void handleClick(Player player) {
        ...
        player.sendMessage(Coloring.translateColors("<yellow>" + traitName + " has been " + status + "<yellow>."));
    }
}