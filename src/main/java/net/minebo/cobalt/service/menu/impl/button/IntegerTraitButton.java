...
public class IntegerTraitButton<T> extends Button {
    ...
    public ItemStack build() {
        ...
        if (meta != null) {
            meta.setDisplayName(Coloring.translateColors("<yellow><bold>" + traitName));

            List<String> lore = new ArrayList<>();
            lore.add("");
            lore.add(Coloring.translateColors("<gray>Current Value: <white>" + currentValue));
            lore.add("");
            lore.add(Coloring.translateColors("<green>▲ Left Click: <gray>+" + incrementAmount));
            if (shiftIncrementAmount > 0) {
                lore.add(Coloring.translateColors("<green>▲ Shift + Left Click: <gray>+" + shiftIncrementAmount));
            }
            lore.add(Coloring.translateColors("<red>▼ Right Click: <gray>-" + incrementAmount));
            if (shiftIncrementAmount > 0) {
                lore.add(Coloring.translateColors("<red>▼ Shift + Right Click: <gray>-" + shiftIncrementAmount));
            }
            lore.add("");
            lore.add(Coloring.translateColors("<gray>Range: <yellow>" + minValue + "<gray> - <yellow>" + maxValue));
    ...
    private void handleClick(Player player, boolean increment, int amount) {
        ...
        if (newValue == currentValue) {
            ...
            player.sendMessage(Coloring.translateColors("<red>Already at " + limit + " value (" + currentValue + ")"));
            return;
        }
        ...
        player.sendMessage(Coloring.translateColors("<yellow>" + traitName + " has been " + direction +
